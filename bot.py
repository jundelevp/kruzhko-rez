...
from aiogram.client.default import DefaultBotProperties  # ← ВАЖНО: новая инициализация
...
# Пути
TEMP_DIR = Path("/tmp/video_temp")  # Используем /tmp для Docker
...
async def on_startup():
    ...
    logger.info(f"ID бота: {BOT_TOKEN.split(':')[0] if ':' in BOT_TOKEN else 'не указан'}")
    logger.info(f"Временная директория: {TEMP_DIR.absolute()}")
    
    # Проверяем наличие FFmpeg
    if check_ffmpeg():
        logger.info("✅ FFmpeg готов к работе")
    else:
        logger.error("❌ FFmpeg не найден! Конвертация невозможна")
...
if __name__ == "__main__":
    ...
    if sys.platform == 'linux':
        try:
            import uvloop
            uvloop.install()
            logger.info("✅ Используется uvloop для оптимизации")
        except ImportError:
            logger.info("ℹ️ uvloop не установлен, используем стандартный asyncio")
